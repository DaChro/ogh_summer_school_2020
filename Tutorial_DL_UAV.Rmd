...
author: "OpenGeoHub summer school 2020"
date: "*Aug 17, 2020*"
output: 
  html_document:
    highlight: tango
...
#If not done already, download and unzip the tutorial data:

#download.file("https://uni-muenster.sciebo.de/s/45044pj8yvQsdOJ/download",destfile = "tutorial_data.zip")
...
```{r test_area,echo=F,warning=F,message=F,eval=T}
...
## Training your model/network {#train}
...
plot(diagnostics)+geom_smooth(method = "loess",se=F)
```
...
viewRGB(input_img,layer.name = "input image", quantiles = c(0,1),r=1,g=2,b=3)+mapview(result_map,layer.name="wildrye prediciton", alpha.regions=0.4,na.alpha=0)
```
...
Here, we will only look at one example of how to get a better understanding of what is actually happening to the images in your model. If you want to continue working with Deep Learning, I recommend to look deeper into this field as well, e.g. by reading the chapter 5.4 of [@Chollet]
...
3. Bonus Question: The figure below shows the sketch of the original U-net, as proposed by [@Ronneberger2015]. Compare it to the model we used in the tutorial. Besides the fact that we are using pretrained layers for the contracting path, what is the other major difference in the architecture? 
...
## Actication functions {#activation_functions}
